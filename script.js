...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxmrNDMB7Ha6iIv7eJyHYeiyItLtTCNG-ua-oFIsYTfIATcYslPimkss5YMMwHdB0mQVw/exec';
...
const STUDIO_ONLY_TABLES = [
  ...
  desc: "매해 돌아오는 하루, 가장 사랑스럽게 기억될 오늘.<br>왕관 및 의상, 양말이 스튜디오에 준비되어 있으며<br>거치대 설치형 풍선으로 세팅됩니다.<br>유료 추가 옵션을 통해 헬륨 풍선 연출로 변경 가능합니다.<br><br>※ 셋팅은 테디모먼트의 곰인형으로 준비됩니다. <br>아이가 평소 좋아하는 인형이나 장난감을 가져오셔서 곰인형 자리에 바꿔 셋팅하시면, <br>우리 아이만의 더욱 자연스럽고 특별한 생일 분위기를 완성하실 수 있습니다.",
...
const birthdayDetailHtml = `
  ...
  <div class="detail-section">
    ...
    <p class="table-detail-desc">
      ...
      유료 추가 옵션을 통해 헬륨 풍선 연출로 변경 가능합니다.<br>
      헬륨으로 변경시, 헬륨풍선을 고정할수 있는 무게추와 끈이 동봉됩니다.<br><br>
  ...
  <div class="detail-section">
    ...
    <p class="table-detail-desc">
      ...
      아이가 방문을 열고 나오기 전<br>
      10분 정도만 먼저 일어나 여유 있게 세팅해 주세요.<br>
      헬륨풍선 만들기 생각보다 매우 쉽습니다 :) <br><br>

      헬륨풍선은 체공시간이 있어<br>
      파티 1시간 전 헬륨 주입을 추천드립니다.<br><br>

      ※ 사진 속의 곰인형 셋팅도 예쁘지만, <br>
      아이가 평소 좋아하는 인형이나 소중한 장난감을 함께 배치해주셔서 한컷 더 남겨보세요<br>
...
const STUDIO_OPTIONS = [
  ...
  { name: "반지 케이스 3개", desc: "와인 또는 네이비 컬러 선택 가능", price: 10000, images: ["images/case.jpg","images/case2.jpg"] },
...
STUDIO_OPTIONS.forEach(function(opt) {
  ...
  html += '<div class="card-img-wrapper" onclick="openImageModal(\''
    + opt.name.replace(/'/g, "\\'")
    + '\', '
    + JSON.stringify(opt.images).replace(/"/g, "'")
    + ')">';

  html += '<img src="' + opt.images[0] + '" alt="' + opt.name + '" style="height:8rem;" />';
  html += '<div class="card-img-overlay"><span>클릭하여 확대</span></div>';
  html += '</div>';
...
if (cameraSnapConsent === '비동의') {
  ...
  if (form.ringCase?.checked) total += 10000;
